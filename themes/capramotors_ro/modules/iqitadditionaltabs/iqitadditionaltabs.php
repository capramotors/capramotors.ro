...
class IqitAdditionalTabs extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        Shop::addTableAssociation('iqitadditionaltab', ['type' => 'shop']);
    ...
    {
        $id_tab = (int) Tab::getIdFromClassName('AdminIqitAdditionalTabs');
        if ($id_tab) {
            $tab = new Tab($id_tab);
            $tab->delete();
        }
    ...
    {
        $tab = new Tab();
        $tab->class_name = 'AdminIqitAdditionalTabs';
        $tab->module = $this->name;
        $tab->active = 0;
        $tab->id_parent = (int) Tab::getIdFromClassName('AdminParentModulesSf'); // nie pokaże się w menu
        foreach (Language::getLanguages(false) as $lang) {
            $tab->name[$lang['id_lang']] = 'AdminIqitAdditionalTabs';
        }
        $ok = $tab->add();
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        Media::addJsDef(['iqitModulesAdditionalTabs' => [
            'ajaxUrl' => $this->context->link->getAdminLink('AdminIqitAdditionalTabs', true, [], [
                'ajax' => 1,
            ]),
        ]]);
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('added')) {
            $output .= '<div class="alert alert-success">' . $this->trans('Tab added', [], 'Modules.IqitAdditionalTabs.Admin') . '</div>';
        }
        ...
        if (Tools::isSubmit('saveIqitAdditionalTab')) {
            ...
            if ($iqitAdditionalTab->validateFields(false) && $iqitAdditionalTab->validateFieldsLang(false)) {
                ...
                $output .= '<div class="conf error">' . $this->trans('An error occurred while attempting to save.', [], 'Admin.Notifications.Error') . '</div>';
            }
        }
        ...
        if (Tools::isSubmit('updateiqitadditionaltabs') || Tools::isSubmit('addIqitAdditionalTab')) {
            ...
            if ($id_iqitadditionaltab = Tools::getValue('id_iqitadditionaltab')) {
                $this->fields_form[0]['form']['input'][] = ['type' => 'hidden', 'name' => 'id_iqitadditionaltab'];
    ...
    protected function initForm()
    {
        ...
        $this->fields_form[0]['form'] = [
            'legend' => [
                'title' => $this->trans('New Tab', [], 'Modules.IqitAdditionalTabs.Admin'),
            ],
            'input' => [
                [
                    'type' => 'switch',
